...
#define SMOKE_STACK_SIZE 16384ULL
#define USER_EXEC_PATH_MAX 128ULL
#define USER_EXEC_MIN_VADDR 0x0000000010000000ULL
#define USER_EXEC_MAX_VADDR 0x0000000080000000ULL
#define USER_EXEC_STACK_SIZE 16384ULL
...
static u32 range_ok(u64 start, u64 size) {
    if (size == 0) {
        return 0;
    }

    if (start < USER_EXEC_MIN_VADDR || start >= USER_EXEC_MAX_VADDR) {
        return 0;
    }
...
static u32 file_range_ok(u64 offset, u64 size, u64 file_size) {
    if (offset > file_size) {
        return 0;
    }
...
static u8* read_file(const char* path, u64* out_size) {
    if (out_size) {
        *out_size = 0;
    }
...
static u32 validate_elf(const u8* file, u64 file_size) {
    ...
    return file_range_ok(ehdr->e_phoff, phdr_size, file_size);
}

static void unmap_range(u64 start, u64 size) {
    if (size == 0) {
        return;
    }
...
static u32 map_range(u64 start, u64 size) {
    if (!range_ok(start, size)) {
        return 0;
    }
...
static s32 map_load_segment(const u8* file, u64 file_size, const elf64_phdr_t* phdr, u64* mapped_begin, u64* mapped_end) {
    ...
    if (!file_range_ok(phdr->p_offset, phdr->p_filesz, file_size)) {
        return -1;
    }

    if (!range_ok(phdr->p_vaddr, phdr->p_memsz)) {
        return -1;
    }
    ...
    if (!map_range(phdr->p_vaddr, phdr->p_memsz)) {
        return -1;
    }

    if (phdr->p_filesz > 0) {
        mem_copy((void*)phdr->p_vaddr, file + phdr->p_offset, phdr->p_filesz);
    }
...
static s32 run_path(const char* path, u64* out_exit_code) {
    ...
    u64 file_size = 0;
    u8* file = read_file(path, &file_size);

    if (!file) {
        return -1;
    }

    if (!validate_elf(file, file_size)) {
        kfree(file);
        return -1;
    }
    ...
    if (!map_range(USER_EXEC_STACK_BOTTOM, USER_EXEC_STACK_SIZE)) {
        if (mapped_begin != 0 && mapped_end > mapped_begin) {
            unmap_range(mapped_begin, mapped_end - mapped_begin);
        }
    ...
    print("\n");
    print("user stack = ");
    print_hex64(USER_EXEC_STACK_BOTTOM);
    print(" - ");
    print_hex64(USER_EXEC_STACK_TOP);
    print("\n");
...
static void print_result(const char* command_name, const char* target, u64 exit_code) {
...
static void cmd_exec(const char* args) {
    ...
    run_command("exec", path);
}

void user_mode_init(void) {
    mark_user_range((u64)ring3_user_blob_start, (u64)(ring3_user_blob_end - ring3_user_blob_start));
    mark_user_range((u64)smoke_stack, SMOKE_STACK_SIZE);
    ready = 1;
...
u64 user_mode_stack_bottom(void) {
    return (u64)&smoke_stack[0];
}

u64 user_mode_stack_top(void) {
    return (u64)&smoke_stack[SMOKE_STACK_SIZE];
}
